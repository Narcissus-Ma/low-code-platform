...
smoothDnD.dropHandler = dropHandlers.reactDropHandler().handler // 这一步是为了干嘛？
smoothDnD.wrapChild = false

const appEditorStore = useAppEditorStore()
const { dragStartChange } = appEditorStore
  ...
  mounted() {
    ...
    for (const key in eventEmitterMap) {
      ...
      options[eventEmitterMap[eventKey]] = (props: any) => {
        if (eventKey === 'drag-start') {
          dragStartChange(true)
        }
        if (eventKey === 'drag-end') {
          dragStartChange(false)
        }
